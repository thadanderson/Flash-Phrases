...
function App() {
  ...
  const [assetPrefix, setAssetPrefix] = useState<string>('/cards/');
        ...
        {currentView === 'APP' ? (
            <>
                ...
                <div className={`absolute top-4 transition-all duration-300 z-10 hidden md:flex gap-3 ${isSidebarOpen ? 'left-4' : 'left-16'}`}>
                   <button
                    onClick={togglePlay}
                    className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg shadow-lg backdrop-blur-md border transition-all active:scale-95 group ${
                      isPlaying 
